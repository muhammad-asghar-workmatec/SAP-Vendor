...
Telerik.Web.UI.Widgets.ImagePreviewer.prototype = {
	...
	{
		if (clientParameters._hidePropertiesTab){
			this._hidePropertiesTab();
		}
	},

	_hidePropertiesTab: function(){
		var tab = this._tab.get_tabs().getTab(1);
		if (tab) {
	...
	_openImageEditor: function()
	{
		...
		if (selectedItem != null && (selectedItem.get_permissions() & Telerik.Web.UI.FileExplorerItemPermissions.Upload) && selectedItem.get_type() == Telerik.Web.UI.FileExplorerItemType.File)
		{
			...
			var args = {};
			args.imageSrc = selectedItem.get_path();
	...
	_selectNewItem: function(src){
		var fileBrowser = this.get_browser().get_fileBrowser(),
			selectFileInterval;

		fileBrowser.refresh();
		selectFileInterval = window.setInterval(function() {
			var fileList = fileBrowser.get_fileList();
			if (fileList.findFileByName(src)) {
				fileList.selectFileByName(src);
				window.clearInterval(selectFileInterval);
			}
		}, 50);
	...
	getResult: function()
	{
		var result = [];
		var fileBrowser = this.get_browser().get_fileBrowser();
		var selectedItems = fileBrowser.get_selectedItems();
		...
		if(selectedItems.length > 1)
		{
			for(var i = 0; i < selectedItems.length; i++)
			{
				if(!selectedItems[i].isDirectory())
					result.push(this._getResultFromItem(selectedItems[i]));
			}
		}
		else
		{
			var oRes = this._imageProperties.getModifiedImage() || this._currentItem;
			if(oRes)
				result.push(this._getResultFromItem(oRes));
		}
		return result;
	},

	_getResultFromItem: function (item) {
		if (!item) return;
		...
		result.src = item.get_url ? item.get_url() : item.getAttribute("src", 2);
		if(!result.getAttribute("alt")) {
			result.setAttribute("alt", "");
		}

		if (this._imageProperties.isThumbLinkChecked() && this._thumbOriginalUrl) {
			if (!result.style.borderWidth) {
				result.style.borderWidth = "0px";
			}

			var oDoc = result.ownerDocument,
	...
	_initImageItem: function(item)
	{
		...
		//fix back slashes - in some browsers they don't work
		src = src.replace(/\\/g, "/");
		src = encodeURI(src);
		img.style.border = "0";
...
<div class="imagePreviewer">
	<table  style="width: 100%;">
		<tr>
			<td>
				<telerik:RadToolBar ID="ImagePreviewToolBar" runat="Server" Width="100%">
					<Items>
